...
const getResponse = useCallback(async () => {
  ...
  if (chatbotId === "0") {
    ...
    try {
      ...
      // Remove the "Fetching response..." message on error
      setMessages((prevMessages) =>
        prevMessages.filter((msg) => msg.content !== "Fetching response...")
      );
      setGenerating(false);
      isProcessingRef.current = false; // Reset the flag
      throw error;
...
React.useEffect(() => {
  ...
  if (messages.length > 0) {
    ...
    if (lastMessage.fromSelf && !isProcessingRef.current) {
      // Get a response only if the last message was from the user and not already processing
      getResponse();
    }
  }
