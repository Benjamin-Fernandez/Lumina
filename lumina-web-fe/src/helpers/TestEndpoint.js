...
try {
  ...
  if (!operation) {
    throw new Error(`Operation not found for path: ${path}`);
  }




  // Extract operationId dynamically from schema instead of hardcoding "getResponse"
  ...
  if (operation?.post?.requestBody?.content["application/json"]) {
    ...
    if (!requestBodySchema) {
      throw new Error(`No schema found for requestBody in ${path}`);
    }
    ...
    if (!properties) {
      throw new Error(
        "requestBodySchema.properties is undefined. Check the schema properties."
      );
    }
    // Extract the first key from properties object (e.g., "query" from {query: {type: "string"}})
    const requestBodyQueryKey = Object.keys(properties)[0];
    if (!requestBodyQueryKey) {
      throw new Error(
        "No property keys found in schema. Check the schema properties."
      );
    }
    console.log("Using requestBodyQueryKey:", requestBodyQueryKey);
