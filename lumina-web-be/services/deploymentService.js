...
async deployFunctionApp(functionAppName, zipBuffer, options = {}) {
  try {
    ...
    // Wait for the function to be ready by checking the health endpoint
    console.log(`Verifying function app is ready...`);
    let functionReady = false;
    const maxRetries = 6;
    for (let i = 0; i < maxRetries; i++) {
      try {
        const healthCheck = await axios.get(`${functionUrl}/api/http_trigger`, {
          timeout: 10000,
          validateStatus: () => true // Accept any status code
        });
        // Function is ready if we get any response (even 4xx/5xx means the app is running)
        if (healthCheck.status < 500) {
          functionReady = true;
          console.log(`Function app is ready (status: ${healthCheck.status})`);
          break;
        }
      } catch (healthErr) {
        console.log(`Health check attempt ${i + 1}/${maxRetries} failed, retrying in 10s...`);
      }
    ...
    if (!functionReady) {
      console.log(`Warning: Function app health check did not succeed, but deployment completed`);
    }
